...
Temuan ini sebagian konsisten dengan Sirin (2005) dalam hal dampak negatif SES rendah terhadap IPK absolut. Namun, hipotesis bahwa keberagaman lebih tinggi memperlebar kesenjangan antar-kuartil tidak terkonfirmasi dalam simulasi ini. Mekanisme utama yang menjelaskan temuan ini adalah efek **pembelajaran teman sebaya** (*peer learning*) dalam konteks kelompok belajar heterogen. Kelompok belajar dibentuk berdasarkan pengetahuan awal (*mixed strategy*), bukan berdasarkan SES. Ketika variansi SES tinggi, mahasiswa SES rendah yang mengalami stres kronis lebih tinggi justru mendapat manfaat dari interaksi dengan anggota kelompok yang memiliki pengetahuan lebih tinggi — konsisten dengan konsep *Zone of Proximal Development* (ZPD) Vygotsky (1978). Transfer pengetahuan dari mahasiswa yang lebih kuat ke yang lebih lemah melalui mekanisme `peer_learning_rate` secara efektif mengompensasi sebagian kerugian akademik yang disebabkan oleh stres SES rendah.

Dengan kata lain, model ini menangkap dua gaya yang berlawanan: (1) SES rendah meningkatkan stres dan menurunkan capaian absolut (efek negatif Sirin, 2005), dan (2) kelompok belajar heterogen memungkinkan transfer pengetahuan dari mahasiswa kuat ke mahasiswa lemah (efek positif Vygotsky, 1978). Pada skenario ses_diverse, kedua efek ini saling mengimbangi lebih kuat daripada pada baseline — mengecilkan kesenjangan relatif antar-kuartil meskipun IPK absolut seluruh kohort sedikit menurun. Interpretasi ini didukung oleh fakta bahwa bobot pengetahuan dalam formula IPK (70%) jauh lebih dominan daripada penyelesaian tugas (30%), sehingga manfaat pengetahuan dari peer learning berdampak lebih besar pada IPK akhir dibandingkan kerugian dari stres yang lebih tinggi.
...
### 4.12.1 Mengapa Konsistensi dengan Literatur Tetap Bernilai

Sebagian besar temuan dalam penelitian ini mengkonfirmasi arah hubungan yang sudah dikenal dalam literatur psikologi pendidikan: umpan balik lambat meningkatkan stres (Shute, 2008), kelas besar menurunkan capaian (Hattie, 2008), dan SES rendah berkorelasi negatif dengan IPK (Sirin, 2005). Konsistensi ini bukan kelemahan, melainkan **bagian dari desain validasi**: arah temuan yang sesuai literatur merupakan prasyarat *face validity* yang membangun kepercayaan terhadap model sebelum digunakan untuk eksplorasi skenario yang lebih kompleks.

Kontribusi utama penelitian ini bukan terletak pada penemuan hubungan baru antara variabel pendidikan — hal tersebut sudah banyak didokumentasikan oleh penelitian empiris. Kontribusinya adalah **artefak komputasional**: sebuah prototipe simulasi *open-source* yang dapat direproduksi, dapat dikonfigurasi melalui antarmuka visual, dan memungkinkan perbandingan skenario kebijakan secara virtual sebelum diterapkan di kelas nyata. Nilai tambahnya terletak pada kemampuan model untuk menangkap **efek interaksi non-linear** antar faktor (seperti pada skenario kasus terburuk di Bagian 4.5) yang sulit diprediksi secara intuitif, serta menyediakan **kuantifikasi relatif** antar faktor kebijakan melalui analisis sensitivitas OAT (Bagian 4.9) yang tidak tersedia dari tinjauan literatur konvensional.
